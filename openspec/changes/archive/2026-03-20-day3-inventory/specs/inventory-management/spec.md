...
The system SHALL display all inventory items in a scrollable list grouped by category, with a count per category.
...
The system SHALL allow the user to edit an item's name, quantity, and category.

#### Scenario: User updates quantity
- **WHEN** user taps edit on an item and enters "2 lbs" in the quantity field and saves
- **THEN** item shows "2 lbs" in the inventory list

#### Scenario: User changes category
- **WHEN** user taps edit and selects a different category chip and saves
