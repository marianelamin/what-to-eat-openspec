...
- **THEN** the detail screen displays all fields: large photo at top, name as heading, ingredients list with have/missing indicators, notes, and action buttons (Choose This Meal, Edit, Archive)
...
#### Scenario: Restore succeeds immediately
- **WHEN** the user taps "Restore Meal" on an archived meal's detail screen
- **THEN** the meal is restored to active status immediately and the user is navigated back to the catalog — no confirmation dialog is shown
