...
The system SHALL allow the user to add meal prep items through the Quick Add modal. The modal SHALL include a two-chip toggle (Ingredients / Meal Prep) to select which kind of items are being added. All items in a single Quick Add batch SHALL be the same kind. The default SHALL be "Ingredients". Near-duplicate detection SHALL be scoped to items of the same kind — ingredient and meal prep items with the same name may coexist.
...
#### Scenario: Same name can exist as both ingredient and meal prep
- **WHEN** "ground beef" exists as an ingredient and user Quick Adds "ground beef" as Meal Prep
- **THEN** a new meal prep entry is created rather than restocking the ingredient
